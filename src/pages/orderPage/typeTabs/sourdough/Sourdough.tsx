...
export default function Sourdough({selections, setSelections}: BagelsProps) {
    return (
        <Fragment>
            {selections.map((type, sakuin) => {
                if (type.type === 'sourdough')
                    return <Grid container direction="row"
                                 sx={{padding: 1, backgroundColor: sakuin % 2 > 0 ? 'lightgrey' : '', borderRadius: 2}}
                                 key={type.label + sakuin}>
                        <Grid container item xs={8} textAlign="start" direction="row" justifyContent="space-between"
                              alignContent="center">
                            <Typography>{type.label}</Typography>
                            <div>approx. {type.weight}g</div>
